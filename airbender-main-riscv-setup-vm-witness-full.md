...
>(
    ...
    let mut memory = VectorMemoryImplWithRom::new_for_byte_size(1 << 30, rom_address_space_bound);
...
pub(crate) fn mark_register_use(
    ...
    let read_timestamp = core::mem::replace(
        self.register_last_live_timestamps.get_unchecked_mut(reg_idx as usize),
        write_timestamp,
    );
...
pub(crate) fn mark_ram_slot_use(
    ...
    let read_timestamp = core::mem::replace(
        self.ram_words_last_live_timestamps.get_unchecked_mut(phys_word_idx as usize),
        write_timestamp,
    );
    debug_assert!(read_timestamp < write_timestamp);
    // 省略access_bitmask更新...
